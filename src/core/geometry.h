...
protected:
    ...
    std :: vector< Coordinates >mVertices;
public:
    /// Constructor.
    BasicGeometry();
    ...
    virtual double computeDistanceTo(const Coordinates &point) { return 0; }
    ...
    virtual void computeNormalSignDist(double &oDist, const Coordinates &iPoint) const = 0;
    virtual void computeTangentialSignDist(double &oDist, const Coordinates &iPoint, double &oMinDistArcPos) const = 0;
    virtual void computeLocalCoordinates(FloatArray &oLocCoord, const Coordinates &iPoint) const { OOFEM_ERROR("not implemented."); }
    virtual void giveSubPolygon(std :: vector< Coordinates > &oPoints, const double &iXiStart, const double &iXiEnd) const { OOFEM_ERROR("?"); }
    virtual void giveGlobalCoordinates(Coordinates &oGlobalCoord, const double &iArcPos) const {OOFEM_ERROR("Not implemented.")};
    ...
    virtual void computeIntersectionPoints(Element *element, std :: vector< Coordinates > &oIntersectionPoints) { }

    inline const Coordinates &giveVertex(int n) const { return mVertices [ n - 1 ]; }

    void setVertices(const std::vector<Coordinates> &iVertices) {mVertices = iVertices;}

    void removeDuplicatePoints(const double &iTolSquare);

    void insertVertexFront(const Coordinates &iP) { mVertices.insert(mVertices.begin(), iP); }
    void insertVertexBack(const Coordinates  &iP) { mVertices.push_back(iP); }

    void clear() {mVertices.clear();}

    void translate(const Coordinates &iTrans);

    /// Initializes the Geometry from the const std::shared_ptr<InputRecord>.
    ...
    virtual bool isInside(const Coordinates &point) { return false; }
     ...
     */
    static double computeLineDistance(const Coordinates &iP1, const Coordinates &iP2, const Coordinates &iQ1, const Coordinates &iQ2);
    ...
    virtual void giveBoundingSphere(Coordinates &oCenter, double &oRadius) {OOFEM_ERROR("Not implemented.")};
...
public:
    Line() : BasicGeometry() { }
    virtual ~Line() { }
    Line(const Coordinates &iPointA, const Coordinates &iPointB);

    BasicGeometry *Clone() override { return new Line(*this); }

    double computeDistanceTo(const Coordinates &point) override;
    /// Computes tangential distance to a point

    void computeNormalSignDist(double &oDist, const Coordinates &iPoint) const override { OOFEM_ERROR("not implemented"); }

    double computeTangentialDistanceToEnd(const Coordinates &point);

    void computeTangentialSignDist(double &oDist, const Coordinates &iPoint, double &oMinDistArcPos) const override;

    void computeProjection(Coordinates &answer);
    int computeNumberOfIntersectionPoints(Element *element) override;
    void computeIntersectionPoints(Element *element, std :: vector< Coordinates > &oIntersectionPoints) override;
...
{
public:
    Triangle(const Coordinates & iP1, const Coordinates & iP2, const Coordinates & iP3);
    virtual ~Triangle() { }

    BasicGeometry *Clone() override { return new Triangle(*this); }

    void computeNormalSignDist(double &oDist, const Coordinates &iPoint) const override
    { OOFEM_ERROR("not implemented"); }
    void computeTangentialSignDist(double &oDist, const Coordinates &iPoint, double &oMinDistArcPos) const override
    { OOFEM_ERROR("not implemented"); }

    double getArea();
    void computeBarycentrCoor(Coordinates &answer) const;
    double getRadiusOfCircumCircle();
    void computeCenterOfCircumCircle(Coordinates &answer) const;
     ...
     * @author Erik Svenning
     */
    bool pointIsInTriangle(const Coordinates &iP) const;
...
public:
    ...
    void giveGlobalCoordinates(Coordinates &oGlobalCoord, const double &iArcPos) const override;
    ...
    bool intersects(Element *element) override;
    void computeIntersectionPoints(Element *element, std :: vector< Coordinates > &oIntersectionPoints) override;
    void computeIntersectionPoints(Line *l, std :: vector< Coordinates > &oIntersectionPoints);
    int computeNumberOfIntersectionPoints(Element *element) override;
    bool isOutside(BasicGeometry *bg) override;
    bool isInside(Element *element) override;
    bool isInside(const Coordinates &point) override;
    void printYourself() override;
...
public:
    ...
    void computeLocalCoordinates(FloatArray &oLocCoord, const Coordinates &iPoint) const override;
    double computeLength() const;

    void giveSubPolygon(std :: vector< Coordinates > &oPoints, const double &iXiStart, const double &iXiEnd) const override;
    void giveGlobalCoordinates(Coordinates &oGlobalCoord, const double &iArcPos) const override;
    ...
    void computeIntersectionPoints(Element *element, std :: vector< Coordinates > &oIntersectionPoints) override;
    void computeIntersectionPoints(Line *l, std :: vector< Coordinates > &oIntersectionPoints);
    void computeIntersectionPoints(const PolygonLine &iPolygonLine, std :: vector< Coordinates > &oIntersectionPoints) const;
    void computeIntersectionPoints(const Coordinates &iXStart, const Coordinates &iXEnd, std :: vector< Coordinates > &oIntersectionPoints) const;
...
public:
    ...
    void initializeFrom(const std::shared_ptr<InputRecord> &ir) override;
#if 0
    double computeDistanceTo(Coordinates &point) override;
    const char *giveClassName() const override { return "Circle"; }
    bool intersects(Element *element) override;
    void computeIntersectionPoints(Element *element, std :: vector< Coordinates > *intersecPoints) override;
    void computeIntersectionPoints(Line *l, std :: vector< Coordinates > *intersecPoints) override;
    bool isOutside(BasicGeometry *bg) override;
    bool isInside(Element *element) override;
    bool isInside(const Coordinates &point) override;
#endif
